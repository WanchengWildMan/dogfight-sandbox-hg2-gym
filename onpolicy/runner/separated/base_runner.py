...
class Runner(object):
    def __init__(self, config):
        ...
        self.type_each_agent = np.zeros(self.num_agents)
        # for i, last_agent in enumerate(last_agent_each_type):
        #     type_each_agent[(0 if i == 0 else last_agent_each_type[i - 1] + 1):last_agent + 1] = i
        # for agent_type in range(self.all_args.num_agent_types):
        #     buffer_type.append(SeparatedReplayBuffer(self.all_args, self.envs.observation_space[last_agent_each_type[agent_type]], share_observation_space))
        for agent_id in range(self.num_agents):
            # algorithm
