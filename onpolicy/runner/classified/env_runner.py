...
from onpolicy.runner.classified.base_runner import Runner
import imageio
...
def _o2a(arr):
    return np.array(list(map(lambda x: np.array(x).tolist(), list(arr.tolist()))))
...
class EnvRunner(Runner):
    ...
    def train(self):
        ...
        for agent_type in range(self.num_agent_types):
            ...
            # actor update
            train_info, samples_per_epoch, advantages = self.trainer[agent_type].train(self.buffer[agent_type])
            train_infos.append(train_info)
            samples_per_agent.append(samples_per_epoch)
            self.buffer[agent_type].after_update()
        if self.merged_critic:  # TODO
    ...
    def run(self):
        ...
        for episode in range(episode_st, episodes):
            ...
            for step in range(self.episode_length):
                ...
                obs_each_type = [[] for i in range(self.num_agent_types)]
                rew_each_type = [[] for i in range(self.num_agent_types)]
                for agent_id in range(self.num_agents):
                    agent_type = self.type_each_agent[agent_id]
                    obs_i_arr = list(map(list, obs[:, agent_id]))
                    obs_each_type[agent_type].append(obs_i_arr)
                    rew_each_type[agent_type].append(rewards[:, agent_id])

                obs = obs_each_type
                rewards = rew_each_type
            ...
            self.compute()
            st = time.time()
            # train_infos = [{} for i in range(self.num_agents)]
            train_infos = [{} for i in range(self.num_agent_types)]
            train_infos_each_type = self.train()
            et = time.time()
            print(et - st)
            # post process
            ...
            if episode % self.log_interval == 0 and True:
                if self.env_name == "MPE" or True:
                    ...
                    for agent_type in range(self.num_agent_types):
                        # idv_rews = []
                        start_agent_id = self.last_agent_each_type[agent_type - 1] + 1 if agent_type > 0 else 0
                        info_i_key = {}  # 一个episode内，第i个agent键为key 的 info
                        keys_i = eps_infos[0][start_agent_id].keys()
                        for key in keys_i:
                            if key in preserved_keys:
                                continue
                            info_i_key[key] = []
                        for info in eps_infos:
                            for key in keys_i:
                                if key in preserved_keys:
                                    continue
                                info_i_key[key].append(info[start_agent_id][key])
                        ...
                        for key in info_i_key.keys():
                            if key.endswith("sum"):
                                train_infos[agent_type].update({key: np.sum(info_i_key[key])})
                            elif key.endswith("mean"):
                                train_infos[agent_type].update({key: np.mean(info_i_key[key])})
                # for agent_id in range(self.num_agent_types):
                #     agent_type = self.type_each_agent[agent_id]
                #     start_agent_id = self.last_agent_each_type[agent_type - 1] + 1 if agent_type > 0 else 0
                #     train_infos[agent_id].update({'individual_rewards': np.sum(self.buffer[agent_type].rewards[:, :, agent_id - start_agent_id])})
                #     train_infos[agent_id].update(train_infos_each_type[agent_type])
                #     train_infos[agent_id].update(
                #         {"average_episode_rewards": np.mean(self.buffer[agent_type].rewards[:, :, agent_id - start_agent_id])})
                for agent_type in range(self.num_agent_types):
                    train_infos[agent_type].update({'individual_rewards': np.sum(self.buffer[agent_type].rewards[:, :, 0])})
                    train_infos[agent_type].update(train_infos_each_type[agent_type])
                    train_infos[agent_type].update(
                        {"average_episode_rewards": np.mean(self.buffer[agent_type].rewards[:, :, 0])})
    ...
    def compute(self):
        ...
        '''
        if not self.merged_critic:
            for agent_type in range(self.num_agent_types):  # 智能体一个一个从critic中获取状态值
                self.trainer[agent_type].prep_rollout()
                next_value = self.trainer[agent_type].policy.get_values(self.buffer[agent_type].share_obs[-1],
                                                                        self.buffer[agent_type].rnn_states_critic[-1],
                                                                        self.buffer[agent_type].masks[-1])
                next_value = _t2n(next_value)
                self.buffer[agent_type].compute_returns(next_value, self.trainer[agent_type].value_normalizer)  # 计算折扣回报
        else:
            inps = []
            for agent_id in range(self.num_agents):
    ...
    def warmup(self):
        ...
        for agent_type in range(self.num_agent_types):
            if not self.use_centralized_V:
                share_obs = np.array(list(obs[:, agent_type]))
            start_agent_id = self.last_agent_each_type[agent_type - 1] + 1 if agent_type > 0 else 0
            end_agent_id = self.last_agent_each_type[agent_type] + 1
            self.buffer[agent_type].share_obs[0] = np.repeat(np.expand_dims(share_obs, 1), self.num_agents_each_type[agent_type], axis=1).copy()
            obss = np.array([list(obs[:, start_agent_id:end_agent_id][:, i]) for i in range(self.num_agents_each_type[agent_type])]).transpose([1, 0, 2])
            self.buffer[agent_type].obs[0] = obss.copy()

    '''
        :returns actions to environment
    '''
    ...
    def collect(self, step):
        ...
        for agent_type in np.arange(self.num_agent_types):
            self.trainer[agent_type].prep_rollout()  # to cpu!!!!!
            if self.merged_critic:
                self.central_critic.to("cpu")

            value, action, action_log_prob, rnn_state, rnn_state_critic \
                = self.trainer[agent_type].policy.get_actions(self.buffer[agent_type].share_obs[step],
                                                              self.buffer[agent_type].obs[step],
                                                              self.buffer[agent_type].rnn_states[step],
                                                              self.buffer[agent_type].rnn_states_critic[step],
                                                              self.buffer[agent_type].masks[step])
            ...
            if self.envs.action_space[agent_id].__class__.__name__ == 'MultiDiscrete':
                ...
                action_env = self.policy[agent_type].map_action(action)
        ...
        def thread_first(arr):
            arr_all = []
            for th in range(self.n_rollout_threads):
                actions_each_th = [[] for t in range(self.num_agent_types)]
                for a_t in range(self.num_agent_types):
                    for i in range(len(arr[a_t][th])):
                        actions_each_th[a_t].append(np.array(list(arr[a_t][th][i])))
                arr_all.append(actions_each_th)
            return arr_all

        actions = np.array(thread_first(actions))
        actions_env = np.array(thread_first(actions_env))
        # try:
        #     actions = np.array([[actions[agent_type][a][b] for a in range(self.num_agents)] for b in range(len(actions[0]))])
        # except:
        #     actions = np.array(actions).transpose(1, 0, 2)

        action_log_probs = np.array(thread_first(action_log_probs))
        rnn_states = np.array(thread_first(rnn_states))
        rnn_states_critic = np.array(thread_first(rnn_states_critic))
    ...
    def insert(self, data):
        ...
        share_obs = [[] for th in range(self.n_rollout_threads)]
        for o_each_type in obs:
            o_each_type_arr = np.array(o_each_type).transpose(1, 0, 2)  # ->thread_num,agent_num_type,size_o
            for th in range(self.n_rollout_threads):
                share_obs[th] += list(chain(*o_each_type_arr[th]))
        share_obs = np.array(share_obs)
        last_type = 0
        for agent_type in range(self.num_agent_types):
            if not self.use_centralized_V:
                share_obs = np.array(obs[agent_type])

            actions_tmp = actions[:, last_type:self.last_agent_each_type[agent_type] + 1]
            if actions_tmp.dtype == 'O':
                actions_tmp = _o2a(actions_tmp)

            self.buffer[agent_type].insert(np.repeat(np.expand_dims(share_obs, axis=1), self.num_agents_each_type[agent_type], axis=1),
                                           np.array(obs[agent_type]).transpose(1, 0, 2),
                                           rnn_states[:, last_type:self.last_agent_each_type[agent_type] + 1],
                                           rnn_states_critic[:, last_type:self.last_agent_each_type[agent_type] + 1],
                                           actions_tmp,
                                           action_log_probs[:, last_type:self.last_agent_each_type[agent_type] + 1],
                                           values[:, last_type:self.last_agent_each_type[agent_type] + 1],
                                           np.array(rewards[agent_type]).transpose(1, 0, 2),
                                           masks[:, last_type:self.last_agent_each_type[agent_type] + 1])
            last_type = self.last_agent_each_type[agent_type] + 1
