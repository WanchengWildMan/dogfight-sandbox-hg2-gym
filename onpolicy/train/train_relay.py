...
def parse_args(args, parser):
    parser.add_argument('--scenario_name', type=str, default='MainRelay', help="Which scenario to run on")
    parser.add_argument("--num_landmarks", type=int, default=3)

    parser.add_argument('--num_users', type=int, default=12, help="number of users")
    parser.add_argument('--num_UAVs', type=int, default=6, help="number of UAVs and BS!@!!!!!")
    parser.add_argument("--num_BSs", type=int, default=3, help="number of BSs in UAVs")
    parser.add_argument("--ground_length", type=float, default=500, help="ground length")
    parser.add_argument("--ground_width", type=float, default=500, help="ground width")
    ...
    parser.add_argument("--T", type=float, default=100, help="time period")
    ...
    parser.add_argument("--relay_H", type=float, default=600, help="BS height")
    ...
    parser.add_argument("--FIXED_b", type=bool, default=True, help="fixed band")
    parser.add_argument("--UAV_ALLOC_b", type=bool, default=False, help="=UAV allocate band")
    parser.add_argument("--W_server", type=float, default=0.01, help="weight of server")
    ...
    parser.add_argument("--DVFS", type=bool, default=False, help="if True, DVFS is used and minimizing energy ")
    parser.add_argument("--INTELLIGENT_BS", type=bool, default=True, help="num_UAVs=num_UAVs_real+num_BS, BS is regarded as UAV")
    parser.add_argument("--CROWD", type=int, default=0, help="number of crowds.")
    ...
    parser.add_argument("--CASE", type=int, default=2, help="action cases")
    ...
    parser.add_argument("--POSITION_SAMPLE_STEPS", type=int, default=400000, help="alpha fixed in")

    parser.add_argument("--DEMO", type=bool, default=False, help="DEMO")
    parser.add_argument("--env_name", type=str, default='MainRelayEnv', help="specify the name of environment")
    parser.add_argument("--experiment_name", type=str, default="FIX_R", help="an identifier to distinguish different experiment.")
    parser.add_argument("--merged_critic", type=bool, default=False, help="use merged critic or not ATTENTION")
...
def main(args):
    ...
    if not all_args.DEMO:
        all_args.is_long_decision_agent = np.concatenate([np.zeros(all_args.num_users), np.ones(all_args.num_UAVs)])
    else:
        all_args.is_long_decision_agent = np.concatenate([np.ones(all_args.num_UAVs), np.zeros(all_args.num_users)])
