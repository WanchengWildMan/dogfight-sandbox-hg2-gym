...
def get_args():
    ...
    parser.add_argument('--task', type=str, default='CTCE-PPO')
    ...
    parser.add_argument('--scenario_name', type=str, default='MyEnvCTCE', help="Which scenario to run on")
    ...
    parser.add_argument("--experiment_name", type=str, default=f"CTCE_W_server=0.001{num_users},{num_UAVs},{num_BSs},{WIDTH}m", help="an identifier to distinguish different experiment.")
    ...
    parser.add_argument("--user_name", type=str, default="wanchengwildman", help="use wandb")
...
def test_ppo(args=get_args()):
    args.num_agents = args.num_UAVs + args.num_users
    env = Env(0, args)
    args.state_shape = env.share_observation_space[0].shape or env.share_observation_space.shape
    args.action_shape = env.share_action_space[0].shape or env.share_action_space.shape
    args.max_action = env.share_action_space[0].high[0] or env.share_action_space.high[0]

    num_agents = args.num_agents
    args.num_agent_types = 2
    args.num_agents_each_type = np.array([args.num_users, args.num_UAVs])  # short,long
    args.is_long_decision_agent = np.concatenate([np.zeros(args.num_users), np.ones(args.num_UAVs)])
    ...
    train_envs = SubprocVectorEnv(
        [lambda: Env(_, args) for _ in range(args.training_num)], norm_obs=False
    )
    # test_envs = gym.make(args.task)
    test_envs = SubprocVectorEnv(
        [lambda: Env(_, args) for _ in range(args.test_num)],
        norm_obs=False,
    ...
    policy = PPOPolicy(
        ...
        action_space=env.share_action_space[0],
